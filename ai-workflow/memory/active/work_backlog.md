...
- [x] **TASK-002-5-a: Provider core + chat real forward** — provider/{base,openai_compat,anthropic,registry,router,deps}.py (BaseProvider Protocol + OpenAI/Ollama/vLLM unified impl + Anthropic native with system lift + max_tokens default + input/output tokens conversion + Registry with from_config factory + Router with prefix parse + ambiguous/no-provider errors + find_by_type/find_default + get_provider_registry dep) + models.py 보강 (ProviderType, ProviderTestRequest/Result, ProviderCreateRequest, ProviderRecord, ProviderInfo, ModelsRefreshResult) + chat_completions.py mock path 제거 (MOCK_HEADER/_build_mock_completion/_estimate_tokens 삭제) + ProviderRouter.invoke 통합 + 400 ambiguous / 502 upstream / 503 no_provider_available 분기 + proxy/app.py app.state.provider_registry + lifespan registry.aclose. tests/test_chat_completions.py 갱신 (mock 제거 6) + test_chat_completions_forwarding.py 신규 (respx 7). Cumulative LOC 3987 (82 passed). ruff/mypy/pytest all green. commit pending in 2026-06-26 cycle.
